...
def test_normalizer():
    # Create dummy data
    obs_data = torch.randn(1000, 128, 9, 2)  # Example shape for observations
    action_data = torch.randn(1000, 128, 2)  # Example shape for actions
    ...
    # Normalize and unnormalize the data
    normalized_data = normalizer.normalize({'obs': obs_data, 'action': action_data})
    unnormalized_data = normalizer.unnormalize(normalized_data)

    # Assert that the original data and unnormalized data are close
    for key in ['obs', 'action']:
        assert torch.allclose(unnormalized_data[key], {'obs': obs_data, 'action': action_data}[key], atol=1e-7)
