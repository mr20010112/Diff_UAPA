...
from diffusion_policy.common.pref_image_replay_buffer import Pref_ImageReplayBuffer
...
class Pbrl_Hdf5ImageDataset(BaseImageDataset):
    def __init__(self,
        ...
        obs_keys = dataset_1.lowdim_keys + dataset_1.rgb_keys
        ...
        self.pref_replay_buffer = Pref_ImageReplayBuffer.create_empty_numpy()
        ...
        if load_dir is None:
            for _ in range(N):
                ...
                episode_1 = dataset_1.replay_buffer.get_episode(idx_1, keys=['action', 'reward'], copy=False)
                obs_1 = dataset_1.replay_buffer.get_episode(idx_1, keys=obs_keys, copy=False)
                episode_2 = dataset_2.replay_buffer.get_episode(idx_2, keys=['action', 'reward'], copy=False)
                obs_2 = dataset_2.replay_buffer.get_episode(idx_2, keys=obs_keys, copy=False)

                # episode = replay_buffer_1.get_episode(idx_1, copy=False)

                # Equal length processing for episode 1
                episode_1_len = len(episode_1['reward'])
                if episode_1_len >= sequence_length:
                    ...
                    for key in obs_1.keys():
                        obs_1[key] = obs_1[key][start:start + sequence_length]
                else:
                    for key in episode_1.keys():
                    ...
                    for key in obs_1.keys():
                        obs_1[key] = np.pad(obs_1[key], 
                                            ((0, sequence_length - episode_1_len),) + ((0, 0),) * (obs_1[key].ndim - 1),
                                            mode='edge')

                # Equal length processing for episode 2
                episode_2_len = len(episode_2['reward'])
                if episode_2_len >= sequence_length:
                    ...
                    for key in obs_2.keys():
                        obs_2[key] = obs_2[key][start:start + sequence_length]
                else:
                    for key in episode_2.keys():
                    ...
                    for key in obs_2.keys():
                        obs_2[key] = np.pad(obs_2[key], 
                                            ((0, sequence_length - episode_2_len),) + ((0, 0),) * (obs_2[key].ndim - 1),
                                            mode='edge')
                ...
                self.pref_replay_buffer.add_pref_episode(
                    data = {
                        **{key:(
                            np.array(obs_1[key], dtype=np.float32).transpose((0, 3, 1, 2)) / 255
                            if key in dataset_1.rgb_keys else np.array(obs_1[key], dtype=np.float32)
                        )
                        for key in obs_keys },
                        **{f"{key}_2":(
                            np.array(obs_2[key], dtype=np.float32).transpose((0, 3, 1, 2)) / 255
                            if key in dataset_2.rgb_keys else np.array(obs_2[key], dtype=np.float32)
                        )
                        for key in obs_keys },
                        'action': np.array(episode_1.get('action', []), dtype=np.float32),
                        'action_2': np.array(episode_2.get('action', []), dtype=np.float32),
                    },
                    meta_data={
            ...
            with h5py.File(load_dir, 'r') as f:
                ...
                observation = {key: np.array(pref_data[key], dtype=np.float32) for key in obs_keys}
                observation_2 = {key: np.array(pref_data[f'{key}_2'], dtype=np.float32) for key in obs_keys}
            ...
            for i in range(N):
                self.pref_replay_buffer.add_pref_episode(
                    data={
                        **{key: np.array(observation[key], dtype=np.float32) for key in obs_keys},
                        **{f"{key}_2": np.array(observation_2[key], dtype=np.float32) for key in obs_keys},
                        'action': np.array(action, dtype=np.float32),
                        'action_2': np.array(action_2, dtype=np.float32),
                    },
                    meta_data={
    ...
    def set_beta_priori(self, data_size=100, policy=None):
        ...
        self.beta_model = BetaImageNetwork(data=pref_data,
                                 ...
                                 data_size=data_size, policy=policy)
