...
class TrainDiffusionTransformerLowdimWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        logger.info(f"Training begin:: {stage1_time} seconds")
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.rollout_every) == 0:
                    runner_log = env_runner.run(policy) # runner_log, episode = env_runner.run(policy)
                ...
                # checkpoint
                # if ((self.epoch % cfg.training.checkpoint_every) == 0 and self.epoch >= 1200) or (self.epoch == 0):
