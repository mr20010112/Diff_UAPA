...
class BETLowdimPolicy(BaseLowdimPolicy):
    ...
    def get_pred_loss(
        ...
        if self.state_prior.predict_offsets:
            ...
            selected_offsets = offsets[
                ...
                target_latents.view(-1, *target_latents.shape[2:]).argmax(dim=-1).view(-1)
                if is_soft_target
    ...
    def compute_loss(self, batch):
        ...
        for i in range(len(obs_1)):
            ...
            pred_action_1 = self.predict_action({'obs': obs_1_slide})

            enc_obs_1 = self.obs_encoding_net(obs_1_slide)
            latent_1 = self.action_ae.encode_into_latent(action_1_slide, enc_obs_1)
            pred_latent_1 = self.action_ae.encode_into_latent(pred_action_1['action_pred'], enc_obs_1)

            loss_1 = self.get_pred_loss(
                obs_rep=enc_obs_1.clone(),
                target_latents=latent_1,
            )
            pred_loss_1 = self.get_pred_loss(
                obs_rep=enc_obs_1.clone(),
                target_latents=pred_latent_1,
            )
            ...
            traj_loss_1 = traj_loss_1 - ((loss_1 - pred_loss_1)*torch.tensor(self.gamma**(i*self.horizon), device=self.device))
            
            obs_2_slide = obs_2[i]
            obs_2_slide[:,To:,:] = -2
            action_2_slide = action_2[i]
            pred_action_2 = self.predict_action({'obs': obs_2_slide})

            enc_obs_2 = self.obs_encoding_net(obs_2_slide)
            latent_2 = self.action_ae.encode_into_latent(action_2_slide, enc_obs_2)
            pred_latent_2 = self.action_ae.encode_into_latent(pred_action_2['action_pred'], enc_obs_2)

            loss_2 = self.get_pred_loss(
                obs_rep=enc_obs_2.clone(),
                target_latents=latent_2,
            )
            pred_loss_2 = self.get_pred_loss(
                obs_rep=enc_obs_2.clone(),
                target_latents=pred_latent_2,
            )
            ...
            traj_loss_2 = traj_loss_2 - ((loss_2 - pred_loss_2)*torch.tensor(self.gamma**(i*self.horizon), device=self.device))

        traj_loss_1 = torch.sum(traj_loss_1, dim=-1)
        traj_loss_2 = torch.sum(traj_loss_2, dim=-1)

        mle_loss_1 = -F.logsigmoid(self.beta*(traj_loss_1 - traj_loss_2))
        mle_loss_2 = -F.logsigmoid(self.beta*(traj_loss_2 - traj_loss_1))
