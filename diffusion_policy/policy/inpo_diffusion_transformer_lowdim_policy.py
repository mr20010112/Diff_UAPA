...
class DiffusionTransformerLowdimPolicy(BaseLowdimPolicy):
    ...
    def compute_loss(self, batch, ref_model: TransformerForDiffusion, avg_traj_loss=0.0, stride=1):
        ...
        for _ in range(self.train_time_samples[0]):
            ...
            sigma_t = torch.sqrt(1 - alpha_t).to(self.device)
            ...
            for i in range(len(obs_1)):
                ...
                pred_1_ref = ref_model(noisy_trajectory_1, timesteps, cond_1)
                pred_trajectory_1 = noisy_trajectory_1 - sigma_t.view(-1, 1, 1, 1) * pred_1
                tau_1 = (pred_trajectory_1 - trajectory_1) / sigma_t.view(-1, 1, 1, 1) + noise_1

                pred_2 = self.model(noisy_trajectory_2, timesteps, cond_2)
                pred_2_ref = ref_model(noisy_trajectory_2, timesteps, cond_2)
                pred_action_2 = noisy_trajectory_2 - sigma_t.view(-1, 1, 1, 1) * pred_2
                tau_2 = (pred_action_2 - trajectory_2) / sigma_t.view(-1, 1, 1, 1) + noise_2
                ...
                slice_loss_1 = torch.norm(((pred_1 - tau_1) - (pred_1_ref - tau_1)) * loss_mask_1.type(pred_1.dtype), dim=-1) ** 2 
                slice_loss_2 = torch.norm(((pred_2 - tau_2) - (pred_2_ref - tau_2)) * loss_mask_2.type(pred_2.dtype), dim=-1) ** 2
